<?php 
    ...
    if (empty($name) || !ctype_alpha($nom)) {
        $erreur .= "Le nom doit être renseigné et ne contenir que des lettres <br>";
      }
      if (empty($description) || !ctype_alpha($description)) {
        $erreur .= "La description doit être renseigné et ne contenir que des lettres <br>";
      }
      ...
      if (empty($erreur)) {
        ajout_Produit($hauteur, $longueur, $largeur, $poids, $name, $ID_vendeur, $prix, $description, $image, $quantity);
...
<body>
    <div class="container">
        <h1>Mes produits</h1>
        <form class="product-form" method="POST" action="mes_produitsN.php">
            <? $erreur ?>
