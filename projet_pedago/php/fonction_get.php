...
function built_tab($ID_Liv){
    ...
    if (mysqli_num_rows($res_cmd) == 0) {
        echo '<p id="erreur">Aucune commande pour ce livreur !</p>';
...
function affiche_Contrat($id, $type){
    ...
    if($res_data){
        ...
        echo $blob;
    }else echo "<p id='erreur'>Pas de contrat pour cette personne</p>";


    
}

function stats($ID_Vend){       //form tab[id_prod] = tab_stat[Qvendu][Qtotal][IMG][Nom][Prix]

    global $link;
    $tab_Stats = [];
    $data = [];

    $all_Prod = recu_Produit_By_Vendeur($ID_Vend);

    foreach ($all_Prod as $value) {

        $id = $value['ID'];
        
        $sql_data = "SELECT * from achats WHERE ID_Produit = $id";
        $res_data = mysqli_query($link, $sql_data);
        $obj = mysqli_fetch_object($res_data);
        ...
        if($obj){
            ...
            $tab_Stats["Qvendu"] = $Qvendu;
            $tab_Stats["Qtotal"] = $Qvendu + $value["Quantite"];
            $tab_Stats["IMG"] = $value["Img"];
            $tab_Stats["Nom"] = $value["Nom"];
            $tab_Stats["Prix"] = $value["Prix"] * $Qvendu;
    
            $data[$value['ID']] = $tab_Stats;
            $tab_Stats = [];   
        }
    }
...
function totalVendeur($id_vend){
    /*
    $produits = recu_Produit_By_Vendeur($id_Vend);
    
    foreach($produits as $produit){
        $id = $produit['ID'];
        $req = $connexion->query("SELECT quantite FROM achats WHERE ID_Produit = $id");
    }
    */
...
function statsA(){
    ...
    $req = $connexion->query("SELECT * from vendeur");
    $dataV = $req->fetchAll();

    $i = 0;

    foreach($dataV as $vendeur){
        $id = $vendeur['ID'];
        $dataV[$i]['Prix'] = totalVendeur($id);
        $i += 1;
    }
